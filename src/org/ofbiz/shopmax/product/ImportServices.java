...
public class ImportServices {
    ...
    private static final String csvFileLocation = UtilProperties.getPropertyValue("shopmax.properties", "shopmax.importcsv.location"); 
    private static final String CSV_PRODUCT_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation +"/products_to_be_imported";
    private static final String CSV_CATEGORY_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation+ "/categories_to_be_imported";
    private static final String ERROR_CSV_PRODUCT_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation + "/products_imported_failed";
    private static final String ERROR_CSV_CATEGORY_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation + "/categories_imported_failed";
    private static final String SUCCESS_CSV_PRODUCT_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation + "/products_imported_success";
    private static final String SUCCESS_CSV_CATEGORY_FILE_LOCATION = System.getProperty("ofbiz.home") + csvFileLocation + "/categories_imported_success";
    ...
    public static Map<String, Object> importProductCategoryFromCSV(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            //clearAllCategoryAssociations(delegator);
        ...
        try {
            if (UtilValidate.isNotEmpty(errorMsgList)) {
                ...
                Debug.logError("Failed in importing of the file name " + CSV_CATEGORY_FILE_LOCATION + "/categories.csv", module);
                Debug.logInfo("Moving file from location " + CSV_CATEGORY_FILE_LOCATION + "/categories.csv" + " to location " + SUCCESS_CSV_PRODUCT_FILE_LOCATION, module);
                result.put(ModelService.ERROR_MESSAGE_LIST, errorMsgList);
                FileUtils.moveFileToDirectory(new File(CSV_CATEGORY_FILE_LOCATION + "/categories.csv"), new File(ERROR_CSV_CATEGORY_FILE_LOCATION), true);
            } else {
                //move file to success folder
                Debug.logInfo("Success in importing of the file name " + CSV_CATEGORY_FILE_LOCATION + "/categories.csv", module);
                Debug.logInfo("Moving file from location "+ CSV_CATEGORY_FILE_LOCATION + "/categories.csv" + " to location " + SUCCESS_CSV_CATEGORY_FILE_LOCATION, module);
                FileUtils.moveFileToDirectory(new File(CSV_CATEGORY_FILE_LOCATION + "/categories.csv"), new File(SUCCESS_CSV_CATEGORY_FILE_LOCATION), true);
    ...
    public static Map<String, Object> importProductsFromCSV(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            readAndImportProductData(delegator, errorMsgList, fileLocation);
        } catch (Exception e) {
            e.printStackTrace();
            errorMsgList.add(e.getMessage());
        }
        try {
            if (UtilValidate.isNotEmpty(errorMsgList)) {
                // Move file to error folder
                Debug.logError("Failed in importing of the file name " + fileLocation, module);
                Debug.logInfo("Moving file from location "+fileLocation+ " to location " + ERROR_CSV_PRODUCT_FILE_LOCATION, module);
                result.put(ModelService.ERROR_MESSAGE_LIST, errorMsgList);
                FileUtils.moveFileToDirectory(new File(fileLocation), new File(ERROR_CSV_PRODUCT_FILE_LOCATION), true);
            } else {
                //move file to success folder
                Debug.logError("Failed in importing of the file name " + fileLocation, module);
                Debug.logInfo("Moving file from location "+fileLocation+ " to location " + SUCCESS_CSV_PRODUCT_FILE_LOCATION, module);
                FileUtils.moveFileToDirectory(new File(fileLocation), new File(SUCCESS_CSV_PRODUCT_FILE_LOCATION), true);
                return ServiceUtil.returnSuccess();
            }
        } catch (IOException e) {
            // TODO Auto-generated catch block
    ...
    private static void readAndImportCategoryData(Delegator delegator, List<String> errorMsgList) throws Exception {
        ...
        try {
            beanReader = new CsvBeanReader(new FileReader(CSV_CATEGORY_FILE_LOCATION + "/categories.csv"), CsvPreference.STANDARD_PREFERENCE);
    ...
    private static List<String> prepareProductCategoryValue (CategoryImportBean productCategoryBean, Delegator delegator) {
        ...
        for (int catCtr = 0; catCtr < categoryIds.length; catCtr++) {
            ...
            if (catCtr != 0) {
                ...
                previousCategoryId =  "SHOPMAX_BROWSE_ROOT";
            }
            ...
            try {
                ...
                if (UtilValidate.isEmpty(productCategory)) {
                    delegator.create(delegator.makeValue("ProductCategory", UtilMisc.toMap("productCategoryId", currentCategoryId, "categoryName", categoryName, "showInFlyout", showOnHomePage, "productCategoryTypeId", "CATALOG_CATEGORY")), true);
                }
    ...
    private static void readAndImportProductData(Delegator delegator, List<String> errorMsgList, String fileLocation) throws Exception {
        ICsvBeanReader beanReader = null;
...
//                }
    ...
    public static Map<String, Object> importProductsFromMultipleCSVs(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Map<String, Object> result = new HashMap<String, Object> ();
        List<String> errorMsgList = new ArrayList<String> ();

        File csvContainer = new File(CSV_PRODUCT_FILE_LOCATION);
        for (final File fileEntry : csvContainer.listFiles()) {
            if (fileEntry.isFile()) {
                String fileName = fileEntry.getName();
                if ((fileName.substring(fileName.lastIndexOf('.') + 1, fileName.length()).toLowerCase()).equals("csv")) {
                    //System.out.println("File= " + folder.getAbsolutePath()+ "\\" + fileEntry.getName());
                    String fileLocation = csvContainer.getAbsolutePath() + "/" + fileEntry.getName();
                    GenericValue userLogin = (GenericValue) context.get("userLogin");
                    try {
                        dispatcher.runAsync("importProductsFromCSV", UtilMisc.<String, Object>toMap("fileLocation", fileLocation, "userLogin", userLogin));
    ...
    private static void clearAllCategoryAssociations(Delegator delegator) {
        try {
            delegator.removeAll(delegator.findList("ProductCategoryRollup", null, null, null, null, true));
        } catch (GenericEntityException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
